function CChipPanel(iX,iY,oParentContainer){
    ...
    this._init = function(iX,iY){
        ...
        _oWithdraw = new CTextButton(73,-40,s_oSpriteLibrary.getSprite("but_clear_bet"), TEXT_WITHDRAW, PRIMARY_FONT, "#fff", 24,_oContainer);
        _oWithdraw.addEventListener(ON_MOUSE_UP,this._onButWithdrawRelease,this);
        _oDeposit = new CTextButton(73,-80,s_oSpriteLibrary.getSprite("but_clear_bet"), TEXT_DEPOSIT, PRIMARY_FONT, "#fff", 24,_oContainer);
        _oDeposit.addEventListener(ON_MOUSE_UP,this._onButDepositRelease,this);
    ...
    this.refreshMoney = function(){
        _oTextMoney.refreshText(Number(s_iCurMoney).toFixed(3)+" "+TEXT_CURRENCY);
    };
    ...
    this._onStartRace = function(){
        if ($("#money").val() - s_iCurMoney > 0) $("#startround").click();
        document.getElementById("startround").setAttribute ("betamount", $("#money").val() - s_iCurMoney);
    ...
    this._onButWithdrawRelease = function () {
        document.getElementById("withdraw").value = s_iCurMoney;
