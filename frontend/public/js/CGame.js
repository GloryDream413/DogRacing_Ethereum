function CGame(iTotBet) {
    ...
    this.greyhoundPhotofinish = function(iIndex,szName){
        ...
        if(_iGreyhoundArrived < 4){
            ...
            s_iCurMoney += parseFloat(_iWin.toFixed(2));
            document.getElementById("endround").setAttribute("earning", _iWin * 0.965);
            document.getElementById("endround").setAttribute("roundfee", _iWin * 0.035);
            if(_bWin || _iWin > 0){
                s_iCurMoney += parseFloat(_iWin.toFixed(2));
                //s_iCurMoney = parseFloat(s_iCurMoney.toFixed(2));
                s_iGameCash -= parseFloat(_iWin.toFixed(2));
