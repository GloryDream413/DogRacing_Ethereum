...
const HashPackConnectModal = React.forwardRef((props, ref) => {
    const { pairingString, connectedAccount, onClickConnectHashPack, onClickCopyPairingStr, onClickDisconnectHashPack } = {...props}
    return (
        <div className="hashpack-connect-container" ref={ref}>
            {
                !connectedAccount &&
                <div className="hashpack-connect-wrapper">
                    ...
                    <Row className="pair-with-code-wrapper" style={{display: 'flex', alignItems: "center"}}>
                        <Input value={pairingString} readOnly={true} />
                        <Button onClick={() => onClickCopyPairingStr()} style={{padding: 15}}>Copy</Button>
                    </Row>
