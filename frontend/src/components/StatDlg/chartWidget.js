...
function CustomPieChart() {
    ...
    return (
        <div className="overflow-x-auto text-white block sm:flex justify-center  w-full border-t-0 border-2 border-white  flex-col ">
            <div className="w-full justify-between px-10 pt-5 pb-0 mb-0">
                <div className="text-light text-xl text-[#8b1832]" style={{textAlign: 'center'}}>{`Win Rate: ${Number(49.96 - Number(randomDelta)).toFixed(2)}%`}</div>
                <br />
                <div className="text-light text-xl text-[#8b1832]" style={{textAlign: 'center'}}>{`Loss Rate: ${Number(50.04 + Number(randomDelta)).toFixed(2)}%`}</div>
            </div>
            <PieChart
